...
# @Last Modified time: 2023-01-09 17:09:08
import numpy as np
import pandas as pd
...
for i, g in enumerate(['adn', 'lmn']):
    ...
    for j, s in enumerate(Pwak[g].keys()):
        ...
        scatter(tmp[:,0], tmp[:,1], c = Pxgb[g][s][0:len(tmp)], alpha = 0.2)
        # scatter(Pwak[g][s][:,0], Pwak[g][s][:,1], color = RGB[g][s])
    ...
    for j, s in enumerate(Pshu[g].keys()):
        ...
        scatter(tmp[:,0], tmp[:,1],'.', c = Pxgb[g][s][len(tmp):], alpha = 0.2)
