...
# @Last Modified time: 2023-03-09 15:29:59
import numpy as np
import pandas as pd
...
datasets = np.hstack([
    np.genfromtxt('/mnt/DataRAID2/datasets_LMN_PSB.list', delimiter = '\n', dtype = str, comments = '#'),
    np.genfromtxt('/mnt/DataRAID2/datasets_LMN_ADN.list', delimiter = '\n', dtype = str, comments = '#')
    ])    
# On Razer
...
# for s in ['LMN-PSB/A3019/A3019-220701A']:
    ...
    if len(lmn)>7:
        ...
        frequency = 1250.0
        binsize = 0.05
        lfp = data.load_lfp(channel=channels[data.basename.split('-')[0]],extension='.eeg',frequency=1250)
        ...
        signal = pyna.eeg_processing.bandpass_filter(lfp, 40.0, 100.0, 1250, order=3)
        signal = signal.restrict(sws_ep)
        lfp = lfp.restrict(sws_ep)
