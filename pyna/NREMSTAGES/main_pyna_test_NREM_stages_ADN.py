...
# @Last Modified time: 2023-03-16 12:32:40
import numpy as np
import pandas as pd
...
# On Razer
# datasets = np.genfromtxt('/media/guillaume/LaCie/datasets_LMN_adn.list', delimiter = '\n', dtype = str, comments = '#')
...
for s in datasets:
    ...
    adn = spikes.getby_category("location")['adn'].getby_threshold('SI', 0.2)
    spikes = spikes.getby_category("location")['lmn'].getby_threshold('SI', 0.1).getby_threshold('halfr', 0.5)
    
    lmn = spikes.index

    if len(adn) > 5 and len(lmn) > 5:
        ...
        # # MUA    
        binsize = 0.02
        total = adn.count(binsize, sws_ep)
        total = total.sum(1)
        ...
        rates_adn.append(
            pd.DataFrame(
                {
                    'nrem2':adn.restrict(nrem2_ep).rates,
                    'nrem3':adn.restrict(nrem3_ep).rates
                }))

        mua_adn[data.basename] = {
            'nrem2':adn.count(0.05, nrem2_ep).sum().sum() / nrem2_ep.tot_length("s"),
            'nrem3':adn.count(0.05, nrem3_ep).sum().sum() / nrem3_ep.tot_length("s")
        }
...
rates_adn = pd.concat(rates_adn)

mua_adn = pd.DataFrame(mua_adn).T

figure()
subplot(131)
# [plot([0, 1], v, 'o-') for v in rates_adn.values]
hist(rates_adn["nrem2"] / rates_adn["nrem3"], 30)
xlim(0, 2)
subplot(132)
hist(mua_adn["nrem2"] / mua_adn["nrem3"])
xlim(0, 2)
subplot(133)
