...
# @Last Modified time: 2023-03-02 14:03:07
...
datasets = {k:np.genfromtxt(os.path.join(data_directory,'datasets_'+k.upper()+'.list'), delimiter = '\n', dtype = str, comments = '#') for k in structs}

coefs_mua = {k:{e:[] for e in ['wak', 'sws', 'rem']} for k in structs}
coefs_pai = {k:{e:[] for e in ['wak', 'sws', 'rem']} for k in structs}

pairs_info = {k:pd.DataFrame(columns = ['offset', 'session']) for k in structs}
...
for k in structs:
    for s in datasets[k]:
        ...
        try:
            ...
            rem_ep = np.atleast_2d(tmp["SleepStateEpisodes"]["ints"]["REMepisode"])
