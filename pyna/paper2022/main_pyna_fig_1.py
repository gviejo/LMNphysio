...
outergs = GridSpec(2,1, figure = fig, height_ratios = [0.4, 0.5], hspace = 0.3)

#####################################
gs1 = gridspec.GridSpecFromSubplotSpec(1,3, subplot_spec = outergs[0,0], width_ratios = [0.2, 0.1, 0.5])
...
for i, (p, n) in enumerate(zip(paths, names)):
	...
	ylabel('REM corr. (r)')
	#title(n)
	text(1.2, 1.2, n, horizontalalignment='center', verticalalignment='center', transform=gca().transAxes, fontsize = fontsize)
	#legend(handlelength = 0.3)
	legend(handlelength = 0.4, loc='center', bbox_to_anchor=(0.2, 0.86, 0.5, 0.5), framealpha =0)
	ax = gca()
	aspectratio=1.0
...
savefig("/home/guillaume/LMNphysio/figures/figures_paper_2022/fig1.pdf", dpi = 200, facecolor = 'white')
#show()