...
# @Last Modified time: 2023-02-07 15:47:39
import numpy as np
import pandas as pd
...
binsizes = {0:0.01, stds[1]:0.1}
windowsizes = {0:0.2, stds[1]:2.0}
...
for i,e in enumerate(coefs_pai.keys()): # epoch 
    ...
    for k, s in enumerate(coefs_pai[e].keys()): # Std
        betaratio[e][s] = {}
        betaneg = coefs_pai[e][s].loc[-0.5:0].abs().sum(0)
        betapos = coefs_pai[e][s].loc[0:0.5].abs().sum(0)
