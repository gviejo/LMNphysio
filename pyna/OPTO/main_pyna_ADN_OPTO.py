...
# @Last Modified time: 2024-09-17 11:08:53
import numpy as np
import pandas as pd
...
datasets = datasets['opto']['opto_adn_psb']
...
savefig(os.path.expanduser("~/Dropbox/LMNphysio/summary_opto/fig_ADN_OPTO.png"))
...
for i, e in enumerate(['sws', 'opto']):
    plot(np.random.randn(len(corr2))*0.1+np.ones(len(corr2))*i, corr2[e], 'o')
ylim(-1, 1)
...
savefig(os.path.expanduser("~/Dropbox/LMNphysio/summary_opto/fig_ADN_OPTO_CORR.png"))
show()
