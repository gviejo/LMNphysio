...
# for s in ['LMN-PSB/A3010/A3010-210324A']:
    ...
    if os.path.isdir(os.path.join(path, "pynapplenwb")):
        ...
        if len(tokeep) > 5:
            ...
            tmp = np.cumsum(tmp) + wake_ep.start[0]

            eps = nap.IntervalSet(start = tmp[0:-1], end = tmp[1:])
            eps = eps.intersect(wake_ep)

            states = np.random.randint(0, 3, len(eps))
            ...
            for n in spikes.keys():
                ...
                for j in range(len(glms)):
                    tmp.append(spikes_glm[j][n].restrict(eps.loc[np.where(states==j)[0]]).index.values)
                tmp = np.sort(np.hstack(tmp))
                spikes_test[n] = nap.Ts(tmp)

            spikes_test = nap.TsGroup(spikes_test, time_support = wake_ep)
            ...
            figure()
            plot(hmm.Z)
            plot(nap.Tsd(t=eps.get_intervals_center().index.values, d=states))
