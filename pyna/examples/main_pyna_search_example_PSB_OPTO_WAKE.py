...
# @Last Modified time: 2023-11-01 15:46:27
import scipy.io
import sys, os
import numpy as np
import pandas as pd
...
path = os.path.join(data_directory, 'OPTO/A8000/A8054/A8054-230719A')
...
nhd = np.array(list(set(spikes.keys()) - set(hd)))

tokeep = spikes.keys()

opto_ep = nap.load_file(os.path.join(path, 'A8054-230719A_opto_ep.npz'))
...
pehd = nap.compute_perievent(spikes[hd], opto_ep.starts, (-4, 14))
...
penhd = nap.compute_perievent(spikes[nhd], opto_ep.starts, (-4, 14))
...
datatosave = {
	'hd':pehd[9],
	'nhd':penhd[11],
	'tc':tuning_curves[[9,11]]
	}

import _pickle as cPickle

filepath = os.path.join(os.path.expanduser("~"), 'Dropbox/LMNphysio/data/DATA_FIG_PSB_WAKE_{}.pickle'.format(os.path.basename(path)))
