...
# @Last Modified time: 2023-08-24 11:51:52
import numpy as np
import pandas as pd
...
gs22 = gridspec.GridSpecFromSubplotSpec(
    2, 3, subplot_spec=outergs[1, 0], width_ratios=[0.12, 0.15, 0.1], wspace=0.3,
    height_ratios = [0.5, 0.2]
)  # , hspace = 0.5)
...
gs_raster = gridspec.GridSpecFromSubplotSpec(
    3,
    1,
    subplot_spec=gs22[0, 0],
    hspace=0.25,
    height_ratios=[0.5, 0.5, 0.4],
    wspace=0.1,
)
...
gs2 = gridspec.GridSpecFromSubplotSpec(1, 1, 
    subplot_spec=gs22[0, 1], wspace=0.7)
...
datahmm = cPickle.load(
    open(os.path.join(dropbox_path, "GLM_HMM_A5043-230302A_18-08-2023.pickle"), "rb")
)


#############################
# MAnifolds
gs_manifold = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=gs22[0, 2])
...
for i, (p, n) in enumerate(zip(paths, names)):
    ...
    xlim(-0.4, 1.4)
    # print(scipy.stats.ttest_ind(y["rem"], y["sws"]))
    print(scipy.stats.wilcoxon(y["ep0"], y["ep1"]))
