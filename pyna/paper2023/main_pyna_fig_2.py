...
# @Last Modified time: 2022-12-29 17:12:54
import numpy as np
import pandas as pd
...
outergs = GridSpec(2,1, figure = fig, height_ratios = [0.3, 0.5], hspace = 0.4)
...
exs = { 'wak':nap.IntervalSet(start = 9968.5, end = 9987, time_units='s'),
        ...
        'sws':nap.IntervalSet(start = 5896.50, end = 5898.25, time_units = 's')
        }
...
for i, ep in enumerate(['wak', 'sws']):
	for j, st in enumerate([psb, lmn]):
		...
		if ep == 'wak':			
			tmp = smoothAngle(angle, 200).restrict(exs[ep])
			plot(tmp, linewidth = lwd, color = (0.4, 0.4, 0.4), label = 'Head-direction')
			tmp2 = angle_wak
			tmp2 = smoothAngle(tmp2, 4)
			tmp2 = tmp2.restrict(exs[ep])
			plot(tmp2, '--', linewidth = lwd, color = 'gray', alpha = alp) 
			if j == 1:
				legend(frameon=False, handlelength = 1, bbox_to_anchor=(0.7,-0.1))
		...
		if ep == 'sws':
			...
			plot(tmp2,'--', linewidth = lwd, color = 'gray', alpha = alp, label = "Decoded head-direction")
			if j == 1:
				legend(frameon=False, handlelength = 1, bbox_to_anchor=(1.2,-0.1))

		if i == 0 and j == 1:			
			plot(np.array([exs[ep].end[0]-1, exs[ep].end[0]]), [0, 0], linewidth = 1, color = 'black')
			xlabel('1s', horizontalalignment='right', x=1.0)
		if i == 1 and j == 1:			
			plot(np.array([exs[ep].end[0]-0.1, exs[ep].end[0]]), [0, 0], linewidth = 1, color = 'black')
			xlabel('0.1s', horizontalalignment='right', x=1.0)
