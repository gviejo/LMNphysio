...
# @Last Modified time: 2023-06-05 22:22:15
import numpy as np
import pandas as pd
...
for s in ['LMN-ADN/A5002/A5002-200304A']:
    ...
    if os.path.isdir(os.path.join(path, "pynapplenwb")):
        ...
        if len(tokeep) > 7:
            ...
            W, W2 = fit_pop_glm(spikes, newwake_ep, bin_size*10, nt, nbasis)
            Ws.append(W)
            W2s.append(W2)


            spikes2 = nap.randomize.shuffle_ts_intervals(spikes.restrict(wake_ep))
            W, W2 = fit_pop_glm(spikes2, newwake_ep, bin_size*10, nt, nbasis)
            Ws.append(W)
            W2s.append(W2)
            ...
            hmm.fit(spikes, wake_ep, bin_size)
