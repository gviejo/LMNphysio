...
# @Last Modified time: 2023-06-03 15:13:00
import numpy as np
import pandas as pd
...
for s in datasets:
    ...
    k = 'lmn'
    tokeep = list(spikes.getby_category("location")[k].getby_threshold("SI", SI_thr[k]).getby_threshold("rate", 1.0).index)
    ...
    if len(tokeep) > 7:
        ...
        W, W2 = fit_pop_glm(spikes, newwake_ep, bin_size, nt, nbasis)
        Ws.append(W)
        W2s.append(W2)


        spikes2 = nap.randomize.shuffle_ts_intervals(spikes.restrict(wake_ep))
        W, W2 = fit_pop_glm(spikes2, newwake_ep, bin_size, nt, nbasis)
        Ws.append(W)
        W2s.append(W2)
        ...
        ep1 = Z.threshold(0.5).time_support
        ep0 = Z.threshold(0.5, 'below').time_support
...
figure()
subplot(131)
plot(allr['wak'], allr['ep0'], 'o', color = 'red', alpha = 0.5)
m, b = np.polyfit(allr['wak'].values, allr['ep0'].values, 1)
x = np.linspace(allr['wak'].min(), allr['wak'].max(),5)
plot(x, x*m + b)
xlabel('wake')
ylabel('ep0')
r, p = scipy.stats.pearsonr(allr['wak'], allr['ep0'])
title('r = '+str(np.round(r, 3)))

subplot(132)
plot(allr['wak'], allr['ep1'], 'o',  alpha = 0.5)
m, b = np.polyfit(allr['wak'].values, allr['ep1'].values, 1)
x = np.linspace(allr['wak'].min(), allr['wak'].max(), 4)
plot(x, x*m + b)
xlabel('wake')
ylabel('ep1')
r, p = scipy.stats.pearsonr(allr['wak'], allr['ep1'])
title('r = '+str(np.round(r, 3)))
