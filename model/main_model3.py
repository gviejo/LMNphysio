...
# @Last Modified time: 2025-07-14 10:05:44
"""
N LMN -> N ADN 
...
for k in ['adn', 'lmn']:    
	popcoh[k] = {}
	for n, m in zip(['wak', 'sws'], [m_wake, m_sws]):
...
for i, st in enumerate(popcoh.keys()):
	subplot(1,2,i+1)
	plot(popcoh[st]['wak'], popcoh[st]['sws'], 'o')
	r, p = pearsonr(popcoh[st]['wak'], popcoh[st]['sws'])
	m, b = np.polyfit(popcoh[st]['wak'], popcoh[st]['sws'], 1)
