...
ax.scatter(xs = ump_adn[~np.isnan(H),0], ys = ump_adn[~np.isnan(H),1], zs = 1, c = RGB[~np.isnan(H)], marker = '.', alpha = 0.9, linewidth = 0, s= 40)
...
ax.scatter(ump_lmn[~np.isnan(H),0], ump_lmn[~np.isnan(H),1], zs = 0, c = RGB[~np.isnan(H)], marker = '.', alpha = 0.9, linewidth = 0, s= 50, zorder = 1)

# ax.view_init(azim=30, elev = 35)

ax.text(15, -1, 0, "LMn", fontsize = 12, fontweight='bold')

ax.text(0, 12, 1, "ADn", fontsize = 12, fontweight='bold')

ax.text(5, -2, 0.5, "?", fontsize = 15, fontweight='bold')
