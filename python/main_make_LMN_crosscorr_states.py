...
for s in datasets[0:-2]:
	...
	cc_sws = compute_CrossCorrs(spikes, sws_ep, 0.25, 200, norm=True)
...
# cPickle.dump(datatosave, open(os.path.join(data_directory, 'Data_Analysis_LMN', 'All_crosscor.pickle'), 'wb'))
...
for i, g in enumerate(sess_groups.keys()):
	for j, n in enumerate(sess_groups[g]):
		subplot(13,10,count, projection = 'polar')
