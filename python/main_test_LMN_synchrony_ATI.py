...
mapping 		= []
ccall 			= []
tcurves 		= {i:[] for i in range(3)}
ahvcurves 		= []
cc_sync 		= []
cc_async 		= []
ahv_sync 		= []
ahv_async 		= []
tcurves_sync 	= {i:[] for i in range(3)}
tcurves_async 	= {i:[] for i in range(3)}
peaks 			= []
...
for s in ['LMN-ADN/A5002/A5002-200304A']:
	...
	# TUNING CURVEs
	######################
	tcurve, velocity, bins_velocity = computeLMNAngularTuningCurves(spikes, position['ry'], wake_ep, 41)	
	for k in tcurve.keys():
		tcurve[k] 			= smoothAngularTuningCurves(tcurve[k], 10, 2)
		tcurve[k].columns	= pd.Index(neurons)
	tokeep, stat 	= findHDCells(tcurve[1], z = 10, p = 0.001)
	
	peak 			= pd.Series(index=tcurve[1].columns,data = np.array([circmean(tcurve[1].index.values, tcurve[1][i].values) for i in tcurve[1].columns]))
	hd_neurons 		= tokeep
	for k in tcurve.keys():
		tcurve[k] 			= tcurve[k][hd_neurons]
	...
	# ccslow = ccall.rolling(window=50, win_type='gaussian', center= True, min_periods=1).mean(std = 20)

	# ccmod = (ccall - ccslow)/ccall.std(0)
	...
	tcurve_s 	= {i:pd.DataFrame(columns = cc.columns) for i in range(3)}
	tcurve_a 	= {i:pd.DataFrame(columns = cc.columns) for i in range(3)}
	...
	for p in cc.columns:
		...
		tc, velocity, bins_velocity = computeLMNAngularTuningCurves(dec_spikes, position['ry'], wake_ep, 41)
		for k in tc.keys():
			tc[k] 	= smoothAngularTuningCurves(tc[k], 10, 2)
	...
	######################
	for k in range(3):
		tcurves[k].append(tcurve[k])
		tcurves_sync[k].append(tcurve_s[k])
		tcurves_async[k].append(tcurve_a[k])
...
for k in range(3):
	tcurves[k]	= pd.concat(tcurves[k], 1)
	tcurves_sync[k] 	= pd.concat(tcurves_sync[k], 1)
	tcurves_async[k] 	= pd.concat(tcurves_async[k], 1)
...
idx = diffs.abs().sort_values().index.values

ctcurves_sync = {k:[] for k in range(3)}
ctcurves_async = {k:[] for k in range(3)}
ctcurves = {k:[] for k in range(3)}
for k in range(3):
	for p in tcurves_sync[k].columns:
		# tmp = tcurves[p[1]] - tcurves_sync[p]
		x = tcurves[1][p[1]].index.values - tcurves[1][p[1]].index[tcurves[1][p[1]].index.get_loc(peaks[p[1]], method='nearest')]
		x[x<-np.pi] += 2*np.pi
		x[x>np.pi] -= 2*np.pi		
		tmp = pd.Series(index = x, data = tcurves_sync[k][p].values).sort_index()
		ctcurves_sync[k].append(tmp.values)
		tmp = pd.Series(index = x, data = tcurves_async[k][p].values).sort_index()
		ctcurves_async[k].append(tmp.values)
	for n in tcurves[k].columns:
		x = tcurves[1][n].index.values - tcurves[1][n].index[tcurves[1][n].index.get_loc(peaks[n], method='nearest')]		
		x[x<-np.pi] += 2*np.pi
		x[x>np.pi] -= 2*np.pi		
		tmp = pd.Series(index = x, data = tcurves[k][n].values).sort_index()
		ctcurves[k].append(tmp.values)
	ctcurves_sync[k] = pd.DataFrame(index = np.linspace(-np.pi, np.pi, tcurve[1].shape[0]+1)[0:-1], data = np.array(ctcurves_sync[k]).T, columns = tcurves_sync[k].columns)	
	ctcurves_async[k] = pd.DataFrame(index = np.linspace(-np.pi, np.pi, tcurve[1].shape[0]+1)[0:-1], data = np.array(ctcurves_async[k]).T, columns = tcurves_async[k].columns)	
	ctcurves[k] = pd.DataFrame(index = np.linspace(-np.pi, np.pi, tcurve[1].shape[0]+1)[0:-1], data = np.array(ctcurves[k]).T, columns = tcurves[k].columns)
...
p = np.argmin(diffs - np.deg2rad(30))

for p in diffs.index[10:20]:
	figure(figsize=(10,6))
	subplot(231)
	[plot(tcurves[k][p[1]], ls = linestyle[k], color=colors[k]) for k in range(3)]
	plot(tcurves[1][p[0]], color = 'orange')	
	subplot(232)
	[plot(tcurves_sync[k][p], ls = linestyle[k], color=colors[k]) for k in range(3)]
	title('sync')
	subplot(233)
	[plot(tcurves_async[k][p], ls = linestyle[k], color=colors[k]) for k in range(3)]	
	title('async')
	for k in range(3):import numpy as np
import pandas as pd
import neuroseries as nts
from pylab import *
from wrappers import *
...
ump = UMAP(n_neighbors = 100, min_dist = 1e-6).fit_transform(tmp.T)
# kmeans = KMeans(n_clusters = 5).fit(ump)
# labels = kmeans.labels_
clustering = SpectralClustering(n_clusters = 6).fit(ump)
...
subplot(gs[2,0])
tmp = ahv_sync.loc[-2:2][diffs.abs().sort_values().index].values.T
imshow(scipy.ndimage.gaussian_filter(tmp, 2), aspect = 'auto')
subplot(gs[3,0])
tmp = ahv_async.loc[-2:2][diffs.abs().sort_values().index].values.T
imshow(scipy.ndimage.gaussian_filter(tmp, 2), aspect = 'auto')
...
plot(bins[0:-1], x/(yc+1))
		...
		subplot(2,3,4+k)
		plot(tcurves_sync[k][p], 'o-', color=colors[k], label = 'sync')
		plot(tcurves_async[k][p], '+-', color=colors[k], label = 'async')
		plot(tcurves[0][p[1]], '--', color = colors[k])
		legend()
	show(block=True)
...
for n in peaks.index:
	# n = peaks.index[1]
	idx2 = [p for p in idx if p[1] == n]
	figure()
	for i,k in enumerate([0,2]):
		subplot(1,2,i+1)
		tmp = ctcurves_sync[k][idx2] - ctcurves_async[k][idx2]
		for p in idx2:
			plot(tmp[p], color = hsv_to_rgb([H[p],1,1]))
	show(block=True)
...
for i,p in enumerate(idx2):
	subplot(4,4,i+1)
	for k in range(3):
		plot(ctcurves[k][n], '--', color = 'grey')
		plot(ctcurves_sync[k][p])
		plot(ctcurves_async[k][p])

for k in range(3):
	plot(dtcurves_sync[k][p])
