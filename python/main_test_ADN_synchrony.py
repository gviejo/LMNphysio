...
for s in datasets:
	...
	mwf, maxch = loadMeanWaveforms(path)


	# TO RESTRICT BY SHANK
	if 'A5001' in s:
		spikes 			= {n:spikes[n] for n in np.where(shank==6)[0]}

	mwf 		= mwf[list(spikes.keys())]
	neurons 		= [name+'_'+str(n) for n in spikes.keys()]
	mwf.columns = pd.Index(neurons)
...
ccfast = ccall.rolling(window=40, win_type='gaussian', center= True, min_periods=1).mean(std = 2)
ccslow = ccall.rolling(window=40, win_type='gaussian', center= True, min_periods=1).mean(std = 10)
...
ump = UMAP(n_neighbors = 10, min_dist = 1e-6).fit_transform(tmp.T)
# kmeans = KMeans(n_clusters = 5).fit(ump)
# labels = kmeans.labels_
clustering = SpectralClustering(n_clusters = 2).fit(ump)
...
for p in ccall.columns.values:
	figure(figsize=(10,6))
	subplot(231)
	plot(ccmod[p])
	plot(ccmod2[p])
	# plot(cc_async[p], color = 'red')
	# plot(cc_sync[p], color = 'green')
	subplot(232, projection = 'polar')
	plot(tcurves[list(p)])
	plot(tcurves_sync[p], '--', label = 'synchrone', color = 'green')
	plot(tcurves_async[p], '--', label = 'asynchrone', color = 'red')
	legend()
	subplot(233)
	plot(tcurves[list(p)])
	plot(tcurves_sync[p], '--', label = 'synchrone', color = 'green')
	...
	for i,j,c in zip(p,range(2),['red', 'blue']):
		tmp = meanwavef[i].dropna()
		tmp = tmp.values.reshape(40,int(len(tmp)/40))
		plot(np.arange(0+j*40,40+j*40),tmp+np.arange(tmp.shape[1])*200, color = c)
	subplot(235)
	plot(ahvcurves[list(p)])
	plot(ahv_sync[p], '--', label = 'synchrone')
	plot(ahv_async[p], '--', label = 'asynchrone')		
	subplot(236, projection = 'polar')
	tmp = tcurves[list(p)]
	plot(tmp/tmp.max())
	plot(tcurves_sync[p]/tcurves_sync[p].max(), '--', label = 'synchrone')
	plot(tcurves_async[p]/tcurves_async[p].max(), '--', label = 'asynchrone')
