...
data2 = cPickle.load(open(os.path.join('../../figures/figures_ipn_2020', 'All_crosscor_async.pickle'), 'rb'))
...
tmp2 = tmp.loc[:-50].append(tmp.loc[50:])

times = tmp2.index.values
tmp2 = tmp2 - tmp2.mean(0)
tmp2 = tmp2 / tmp2.std(0)
tmp2 = scipy.ndimage.gaussian_filter(tmp2.T, (1, 1))

imshow(tmp2, aspect = 'auto', cmap = 'jet', interpolation = 'bilinear')
xticks([0, np.where(times==0)[0], len(times)], ['-1.5', '0', '1.5'])
yticks([0, len(pairs)-1], [1, len(pairs)])
# title(epoch)
xlabel("Time lag (ms)")
title("non-REM sleep", fontsize = 14)
...
outergs.update(top= 0.92, bottom = 0.15, right = 0.96, left = 0.03)

savefig("/home/guillaume/Dropbox (Peyrache Lab)/Talks/2020-IPN/fig_ipn_3_8.png", dpi = 200, facecolor = 'white')