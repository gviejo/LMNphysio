...
outergs = GridSpec(1,3, figure = fig, width_ratios = [0.1, 0.75, 0.75], wspace = 0.4)
...
tmp4 = tmp3
times2 = times[np.logical_or(times<-40, times>40)]

imshow(tmp4.T, aspect = 'auto', cmap = 'jet', interpolation = 'bilinear')
# times = cc_sws.index.values[xindex]
xticks([0, np.where(times2==times2[len(times2)//2])[0], len(times2)], [int(times2[0]), "-40/40", int(times2[-1])])
yticks([0, len(pairs)-1], [1, len(pairs)])
# title(epoch)
xlabel("Time lag (ms)")
...
savefig("/home/guillaume/Dropbox (Peyrache Lab)/Talks/2020-IPN/fig_ipn_5_5.png", dpi = 200, facecolor = 'white')