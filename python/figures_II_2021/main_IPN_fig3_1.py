...
for k, n in enumerate(adn):
	...
	if len(spk):
		...
		plot(spk, np.ones_like(spk)*tcurves[n].idxmax(), '|', color = clr, linewidth = 6, markeredgewidth = 2)
...
ylabel("ADN", labelpad = -10, fontsize = 15)
title("Wake", fontsize = 15)
...
yticks([0, 2*np.pi], ["0", "360"])
xticks(list(ep.loc[0].values), ['0', str(int(ep.tot_length('s')))+' s'])
ylabel("LMN", labelpad = -10, fontsize = 15)
...
# title("non-REM sleep", fontsize = 15)
...
outergs.update(top= 0.94, bottom = 0.06, right = 0.98, left = 0.05)

savefig("/home/guillaume/Dropbox (Peyrache Lab)/Talks/2021-II/fig_ipn_3_1.png", dpi = 200, facecolor = 'white')