...
outergs = GridSpec(3,1, figure = fig, height_ratios = [0.6, 0.3, 0.3], hspace = 0.4)#, width_ratios = [0.25, 0.75], wspace = 0.3)

gs_top = gridspec.GridSpecFromSubplotSpec(2,2, subplot_spec = outergs[0,0], width_ratios = [0.3, 0.7], hspace = 0.3)#, height_ratios = [0.2, 0.8], hspace = 0)

####################################################################
# A TUNING CURVES
####################################################################
gs_left = gridspec.GridSpecFromSubplotSpec(6,4, subplot_spec = gs_top[:,0], hspace = 0.1, wspace = 0.1)#, width_ratios = [0.1, 0.5, 0.5, 0.5], height_ratios = [0.2, 0.8], hspace = 0)
...
for k, n in enumerate(neurons):
	...
	if len(spk):		
		...
		if n in [25, 24]:
			...
			plot(spk, np.ones_like(spk)*tcurves[n].idxmax(), '|', color = 'grey', linewidth = 5, markeredgewidth = 1, alpha = 0.05)
plot(angle.restrict(ep), linewidth = 1, color = 'black', alpha = 0.25)
...
t = np.linspace(ep.loc[0, 'start'], ep.loc[0, 'end'], 20)
for t in t: axvline(t, color = 'grey', alpha = 0.5, linestyle = '--', linewidth = 1)
# ####################################################################
# # B SWS
# ####################################################################
subplot(gs_top[1,1])
...
ep = nts.IntervalSet(start = 3.69296e+9, end = 3.69996e+9)
for k, n in enumerate(neurons):
	spk = spikes[n].restrict(ep).index.values
	if len(spk):		
		clr = hsluv.hsluv_to_rgb([tcurves[n].idxmax()*180/np.pi,85,45])
		if n in [25, 24]:
			...
			plot(spk, np.ones_like(spk)*tcurves[n].idxmax(), '|', color = 'grey', linewidth = 5, markeredgewidth = 1, alpha = 0.2)
plot(angle.restrict(ep), linewidth = 1, color = 'black', alpha = 0.25)
# xlim(times[0], times[-1])
# ylim(-1, len(neurons)+1)
ylim(0, 2*np.pi)
...
####################################################################
# B CROSS-CORR SAME DIRECTION
####################################################################
gs_middle = gridspec.GridSpecFromSubplotSpec(1,3, subplot_spec = outergs[1,0], width_ratios = [0.2, 0.4, 0.6])#, height_ratios = [0.2, 0.8], hspace = 0)


subplot(gs_middle[0,1], projection = 'polar')
gca().grid(zorder=0)
for n in [25, 12]:
	clr = hsluv.hsluv_to_rgb([peaks[n]*180/np.pi,85,45])	
	tmp = tcurves[n].values/tcurves[n].max()
	fill_between(tcurves[n].index.values, np.zeros_like(tcurves[n].index.values), tmp, color = clr, alpha = 0.3, linewidth =0, zorder=2)	
	plot(tcurves[n].index.values, tmp, color = clr, alpha = 1, linewidth = 2)
xticks([0, np.pi/2, np.pi, 3*np.pi/2], [])
yticks([])



subplot(gs_middle[0,2])
simpleaxis(gca())

cc = compute_PairCrossCorr(spikes, wake_ep, [25,12], binsize=20, nbins = 1000, norm = False)
cc = cc.rolling(window=100, win_type='gaussian', center= True, min_periods=1).mean(std = 2.0)

fill_between(cc.index.values, np.zeros_like(cc.index.values), cc.values, color = 'darkgreen', alpha = 0.8)
xticks([-10000,-5000, 0, 5000,10000], [-10,-5, 0, 5,10])
yticks([])
...
# ####################################################################
# # C CROSS-CORR DIFF DIRECTION
# ####################################################################
gs_bottom = gridspec.GridSpecFromSubplotSpec(1,3, subplot_spec = outergs[2,0], width_ratios = [0.2, 0.4, 0.6])#, height_ratios = [0.2, 0.8], hspace = 0)

subplot(gs_bottom[0,1], projection = 'polar')
gca().grid(zorder=0)
for n in [25, 24]:
	clr = hsluv.hsluv_to_rgb([peaks[n]*180/np.pi,85,45])	
	tmp = tcurves[n].values/tcurves[n].max()
...
cc = compute_PairCrossCorr(spikes, wake_ep, [25,24], binsize=20, nbins = 1000, norm = False)
cc = cc.rolling(window=100, win_type='gaussian', center= True, min_periods=1).mean(std = 2.0)

fill_between(cc.index.values, np.zeros_like(cc.index.values), cc.values, color = 'darkblue', alpha = 0.8)
xticks([-10000,-5000, 0, 5000,10000], [-10,-5, 0, 5,10])
yticks([])
ylabel("Norm. correlation")
xlabel("Time (s)")
...
savefig("/home/guillaume/Dropbox (Peyrache Lab)/Talks/2021-II/fig_ipn_2_6.png", dpi = 200, facecolor = 'white')
