...
tuning_curves 	= computeAngularTuningCurves(spikes, position['rz'], wake_ep.loc[[0]], 60)

tokeep, stat 	= findHDCells(tuning_curves, z = 20, p = 0.001 , m = 2)

plotTuningCurves(tuning_curves, tokeep)
...
ump = Isomap(n_components = 2, n_neighbors = 50).fit_transform(data.values)

scatter(ump[:,0], ump[:,1], c = sessions,  alpha = 0.4, linewidth = 0, s = 40)
show()
...
for i, n in enumerate(np.unique(sessions)[0:1]):
