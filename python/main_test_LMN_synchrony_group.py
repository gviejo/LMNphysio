...
for s in np.array(datasets)[[6,7,8,9,10,11]]:
	...
	#######################
	# SEARCHING FOR EPOCHS OF HIGH CORRELATION
	#######################
	ccfast = cc.rolling(window=50, win_type='gaussian', center= True, min_periods=1).mean(std = 1)
	...
	idx = ccmod.loc[-3:3].max().sort_values().index.values
	hd_group = np.unique(np.array([list(p) for p in idx[-3:]]))
		
	group_spikes = nts.Ts(t = np.sort(np.hstack([spikes[int(n.split('_')[1])].restrict(wake_ep).index.values for n in hd_group])))

	bins = np.arange(wake_ep.loc[0,'start'],wake_ep.loc[0,'end']-10000, 10000)
	counts = np.zeros((len(bins)-1,5))
	for i in range(5):		
		counts[:,i], _ = np.histogram(group_spikes.index.values, bins+i*2000)

	
	sumcount = pd.Series(index = bins[0:-1]+np.diff(bins)/2, data = counts.sum(1))
	sumcount = sumcount.rolling(window=20, win_type='gaussian', center= True, min_periods=1).mean(std = 2)

	figure()
	ax = subplot(211)
	for i, n in hd_group:
		plot(spikes[int(n.split('_')[1])].restrict(wake_ep).fill_na(peak.loc[n]), '|')
	subplot(212, sharex = ax)
	for i,n in enumerate(hd_group):
		subplot(2,2,i+1,projection='polar')
		plot(tcurve[n])
